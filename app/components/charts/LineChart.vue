    ...
    <div v-if="showLegend && legendItems.length > 0" class="flex items-center justify-center gap-6 mt-6">
      <div v-for="(item, index) in legendItems" :key="index" class="flex items-center gap-2">
        <div
          ...
          :style="{ backgroundColor: legendColors[index] || [chartDefaults.colors.primary, chartDefaults.colors.success][index] }"
        ></div>
...
const mergedOptions = computed(() => {
  const baseOptions = chartDefaults.getLineChartDefaults()
  return {
    ...baseOptions,
    stroke: {
      ...baseOptions.stroke,
      curve: props.curveType === 'smooth' ? 'smooth' : 'straight',
    },
    xaxis: {
      ...baseOptions.xaxis,
